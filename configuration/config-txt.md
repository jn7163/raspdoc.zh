...
`vcgencmd get_config <config>` - 显示特定设置项的参数，例如`vcgencmd get_config arm_freq`。

`vcgencmd get_config int` - lists all the integer config options that are set (non-zero).

`vcgencmd get_config int` - 列出所有参数中包含数字的设置项

`vcgencmd get_config str` - lists all the string config options that are set (non-null).

`vcgencmd get_config str` - 列出所有参数中包含字符串的设置项

Note that there are a few config settings which can't be retrieved using `vcgencmd`.

注意有部分配置参数讲不能被`vcgencmd`检索到。

## File format（文件格式）


`config.txt`在开机时首先被固件读取，所以其格式比较简单。它的每一行遵循一种特定的形式（`选项=参数`），参数可以是数字或字符。使用“#”字符开头可以添加注释，也可以使某一行的配置暂时失效。
...
以MB为单位来表示显存的大小。这项设置将对物理内存进行分割，设定的容量大小将分配分给CPU，而余下的内存容量则分配给GPU。这一项的最小值为`16`，最大值可以是`192`,`448`或者`944`，这取决于你的树莓派实际物理内存大小。显存的默认值为`64`。
...
显存设置太少将导致一些特定功能无法自动加载，因为GPU在低显存无法处理这些任务，如果遇上以上情况，可以尝试增加分配给GPU的内存容量。

使用`gpu_mem_256`, `gpu_mem_512`或`gpu_mem_1024`等参数将让你在不同内存容量的派之间更加灵活地切换内存卡而无需每次更改配置。

### gpu_mem_256（256M显存）

为物理内存是256M的树莓派设置的显存（如果树莓派的内存不是256M，这项设置将忽略），将重载上文对 显存 的大小设置（`gpu_mem`中的设置将暂时失效）。最大值设置为`192`而没有设置默认值。

### gpu_mem_512（512M显存）

为物理内存是512M的树莓派设置的显存（如果树莓派的内存不是512M，这项设置将忽略），将重载上文对 显存 的大小设置（`gpu_mem`中的设置将暂时失效）。最大值设置为`448`而没有设置默认值。


### gpu_mem_1024（1024M显存）
...
为物理内存是1024M的树莓派设置的显存（如果树莓派的内存不是1024M，这项设置将忽略），将重载上文对 显存 的大小设置（`gpu_mem`中的设置将暂时失效）。最大值设置为`944`而没有设置默认值。
...
此项设置为`1`将禁止CPU访问GPU的二级缓存；而且要求相应的内核禁用二级缓存。此项默认值为`0`。
...
此项设置为`1`将禁止调节每500ms；此项默认值为`0`。


### CMA - Dynamic memory split（动态内存分割）
...
此项设置为`1`将在树莓派相机在拍摄时禁用红色的LED指示灯，这对拍摄一些光滑物体时（如玻璃窗）阻止光线的反射很有帮助。

## Onboard analogue audio (3.5mm jack)（板载模拟音频3.5mm插孔）
...
对板载音频输出的一些配置可以改变模拟音频驱动的方式，以及决定是否加载某些特定的功能。

### disable_audio_dither（音频消抖）
...
默认情况下，1.0最低效位元的抖动在被路由到音频输出口时作用于音频流，这将导致某些情况下输出音频有听得见的嘶嘶声，高级Linux声音架构的音量会被设置的比较低，设置此项为`1`将禁止这种抖动作用于音频流。
...
调整模拟音频输出的 位深度 ，默认的位深度位`11`，选择低于`8`的位深度将导致输出没有任何效果的音频，低于`8`的位深度将导致锁相环频率太低，这通常只是作为探究位深度如何影响量化噪声的示例。

## Video(视频)

### Composite video mode options（复合视频模式选项）

#### sdtv_mode（接口标准）

Defines the TV standard used for composite video output over the yellow RCA jack; the default value is `0`.

定义了用于复合视频输出的黄色RCA的接口标准，该项默认值为`0`。
...
此项设置为`1`将禁用复合视频输出时的色同步。输出的将是黑白的图像，但有可能比以前更加锐利。


### HDMI mode options（HDMI模式选项）
...
此项设置为`1`将尝试使HDMI以最大的兼容性进行启动，这等同于设置如下参数。
